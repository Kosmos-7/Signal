...
def construire_prompt_analyse(portfolio, watchlist, contexte, macro_news=None):
    ...
    for s in watchlist.get("stocks", [])[:10]:
        ...
        top10_lines.append(
            f"  #{s['rank']} {s['ticker']} score={s['score']}/100{cross_str}{z_str}{dc_str}{tu_str}{val_str}{fibo_str} — {s.get('justification','')[:100]}{warn_str}"
        )
...
def construire_prompt(portfolio, watchlist, contexte, analyse=None, macro_news=None, regles_auto=None):
    ...
    for s in watchlist.get("stocks", [])[:10]:
        ...
        top10_lines.append(
            ...
            f"{cross_str}{vol_str}{z_str}{dc_str}{tu_str}{val_str}{fibo_str}{dyn_str}{opp_str} — {s.get('justification', '')[:100]}{warn_str}"
        )
...
- Signal est un COMPTE-TITRES ORDINAIRE français : 31,4 % de PFU sur chaque plus-value réalisée à la VENTE.
  ...
  → Conséquence directe : buy & hold long terme est structurellement favorisé. Une vente "neutre"
    (sortir d'une position correcte pour réinvestir ailleurs) perd 31,4 % sur le gain → la position
