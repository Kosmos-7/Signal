...
(function(){
  ...
  function draw(){
    ...
    for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],d=Math.hypot(a.x-b.x,a.y-b.y);if(d<135){x.strokeStyle='rgba(110,170,225,'+((1-d/135)*0.055)+')';x.beginPath();x.moveTo(a.x,a.y);x.lineTo(b.x,b.y);x.stroke();}}
    for(const n of nodes){const tw=0.55+0.45*Math.sin(n.ph);x.fillStyle='rgba(116,182,223,'+(0.06*tw)+')';x.beginPath();x.arc(n.x,n.y,n.r*3.4,0,6.2832);x.fill();x.fillStyle='rgba(165,205,240,'+(0.62*tw)+')';x.beginPath();x.arc(n.x,n.y,n.r,0,6.2832);x.fill();}
